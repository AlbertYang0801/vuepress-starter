...
2. 调用 `add(E e)`方法添加元素时候，会初始化一个为默认长度 10 的数组。
...
3. 当继续添加元素时，底层数组长度已满，此时会执行扩容策略。

4. 以当前数组长度的1.5 倍左右，计算扩容后数组长度。

   通过位运算实现，将长度值向右位移一位，得到当前容量的二分之一左右。（位运算的好处：效率高、节省计算资源）
...
- 变化过程
  ...
  - JDK 1.8 之前，以初始容量 10 创建一个长度为10的数组，类似饿汉式。

  - JDK 1.8 之后，创建一个空数组，类似懒汉式，在第一次添加元素的时候对内部数组进行初始化。
...
底层基于链表实现，双向链表（JDK1.6之前位双向循环链表，JDK1.7以及以后为普通双向链表）。只能顺序访问，可以快速的在链表中间插入和删除元素。
...
- `add(int index,E element)`
  ...
  2. 判断遍历次序。
     ...
     - 若 `index < (size >>1) ` ，则从前遍历找到插入位置的后继节点。
     - 若 `index >= (size >> 1) `，则从后向前遍历找到插入位置的后继节点。
...
- `get(int index)`
  ...
  2. 判断遍历次序。
     ...
     - 若 `index < (size >>1) ` ，则从前向后遍历找到指定位置节点。
     - 若 `index >= (size >> 1) `，则从后向前遍历找到指定位置节点。
...
1. 查找/修改
   ...
   - 单链表查找时只能从头节点开始往后查找，查找某节点前驱时间复杂度为 `O(n)`，查找某节点后继时间复杂度为`O(1)`。

   - 双向链表查找时，查找某节点前驱和后继的时间复杂度都为`O(1)`。查找某个元素的时候还可以使用二分法的思想，从头部节点或者尾部节点开始查找。这样查找和修改的效率可以提高一倍。
...
1. 使用单链表每次查询都要从头开始，使用双链表可以采用 `二分法`，效率至少提高一倍。
...
双向循环链表的尾节点的 next 指向头节点，而头节点的 prev 指向头节点，构成一个环。
...
2. 底层数据结构？
   ...
   LinkedList 底层使用的是双向链表（JDK1.6之前位双向循环链表，JDK1.7为普通双向链表）。
...
3. 插入和删除是否受元素位置的影响？
   ...
   - LinkedList 底层采用双向链表存储。

4. 是否支持快速访问？

   - ArrayList 支持高效的随机元素访问，可通过元素位置快速获取元素对象。
   - LinkedList 不支持高效的随机元素访问。

5. 内存空间占用？

   - ArrayList 的空间浪费主要体现在 list 列表的结尾会预留一定的空间，对应的是底层数组的冗余空间。
   - LinkedList 的空间花费体现在底层数据结构为双向链表上，每个元素都需要保存其直接前驱、直接后继和数据。
...
3. 扩容因子？
   ...
   - 而 Vector 默认构造因子是 2 倍，也可以通过构造方法设置构造因子。

### 迭代器修改数据的问题？

在迭代器内部直接删除数据会报并发异常 `java.util.ConcurrentModificationException`。

在 `AbstractList` 类维护了一个 `modCount` 变量。

```java
protected transient int modCount = 0;
```

集合内部涉及增加删除等操作的时候，都会修改 `modCount` 变量的值。在迭代器中修改数据时，若识别到`modCount` 变量发生异常就会抛出并发异常。
